...
| Drupal 10/11 | [/drupal/](https://onout.org/drupal/) | [How to fix Drupal issues](https://onout.org/drupal/how-to-fix-drupal-issues/) + [Display errors and read logs](https://onout.org/drupal/drupal-display-errors-and-logs/) + [Hire Drupal developer or use SimpleReview](https://onout.org/drupal/hire-drupal-developer-or-use-simplereview/) with Drupal 11.3.8 Docker screenshots and voiced SimpleReview banners | drupal.org/project/ |
...
The repo now has an Astro static-build path for the SEO migration. It preserves the current static HTML sources, generates temporary Astro route files during build, outputs the generated site to `dist/`, normalizes legacy `*.html` URLs back to flat files, and writes `dist/sitemap.xml` from the same route inventory. The Astro root `/` intentionally maps to `simple-review/index.html`, matching the current nginx production root; the Vibers product page is generated at `/vibers/`.
...
python3 scripts/inspect-gsc-onout.py
```

Preview is local on `http://127.0.0.1:4321/`.

`npm run deploy:astro` is the production path: it builds `dist/`, regenerates the combined production sitemap, runs `nginx -t`, reloads nginx, and then runs live smoke checks. It does not use CI because production is served directly from this host.

Production `onout.org` now serves the Astro `dist/` output for:

- `/` → `/root/vibers/dist/index.html` (SimpleReview landing)
- `/vibers/` and `/vibers/blog/*` → `/root/vibers/dist/vibers/...`
- CMS hubs and CMS articles such as `/bitrix/`, `/wordpress/...`, `/webasyst/...` → `/root/vibers/dist/...`
- `/simple-review/*` and `/simple-review-wordpress/*` → `/root/vibers/dist/...`

Legacy onout.org product sections such as `/dao/`, `/wallet/`, `/dex/`, `/blog/` and backend proxy endpoints still use the existing nginx locations. Exact `/vibers/*` API routes continue to proxy to `vibers-feedback` on port `3847`.

The public sitemap remains the combined nginx-served sitemap at `https://onout.org/sitemap.xml`, backed by `/root/vibers/generated/onout-sitemap.xml`, so legacy onout.org URLs and Astro/Vibers URLs stay in one sitemap. Build-only `dist/sitemap.xml` is useful for validating the Astro route inventory but is not the production sitemap yet.

The combined sitemap is generated by `scripts/generate-onout-sitemap.py`. It reads Astro routes from `src/lib/static-html-routes.mjs` for Astro-owned public URLs and scans `/var/www/onout.org` for legacy static sections. `/blog/` intentionally remains the legacy Onout blog; Vibers blog URLs are only emitted under `/vibers/blog/`. Redirecting and internal generated `.html` helper pages are excluded from the public sitemap.

Production smoke checks live in `scripts/smoke-prod-astro.sh`. They validate canonical pages, legacy sections, assets, `/vibers/health`, sitemap contents, and the `/blog/` versus `/vibers/blog/` split.

Google Search Console checks live in `scripts/inspect-gsc-onout.py`. The script uses the local Google service account key, reads `sc-domain:onout.org`, combines live sitemap URLs with Search Analytics pages from the last 90 days, and verifies that every discovered URL returns a final 200. Reports are written under ignored `logs/gsc-onout/`.

Production switch notes:

- nginx config: `/etc/nginx/sites-available/onout.org`
- backup before Astro switch: `/etc/nginx/sites-available/onout.org.bak.2026-05-08-astro-switch`
- backup before GSC redirect fixes: `/etc/nginx/sites-available/onout.org.bak.2026-05-08-gsc-index-fixes`
- baseline/after check logs and screenshots: `logs/prod-astro-switch-20260508/`
...
- Animated banner recording uses `scripts/record-article-banner.js` with a full visible `720x460` crop; inspect a montage before commit so captions, popup text, and progress bars are not accidentally trimmed.
...
- Validate rendered article images through the nginx vhost when possible, not only `file://`: `curl -H "Host: onout.org" http://127.0.0.1/<path>` for assets/pages; for Puppeteer use a local proxy that forwards to `127.0.0.1:80` with `Host: onout.org`.
